...
namespace Findwise.Configuration.TypeEditors
{
    ...
    /// <para>In order to include the base type or sort types alphabetically add <see cref="TypeScheduleTypeEditor.OptionsAttribute"/>.</para>
    /// </summary>
    /// <typeparam name="T">Base class or implemented interface</typeparam>
    public class TypeScheduleTypeEditor<T> : ListBoxEditor
    {
        protected override object[] GetItems(ITypeDescriptorContext context)
        {
            var dataSource = typeof(T).GetDerivedTypes();
            var options = context.PropertyDescriptor.Attributes.OfType<TypeScheduleTypeEditor.OptionsAttribute>().FirstOrDefault();
    ...
    /// <summary>
    /// <para>UI type editor that provides abilty to choose one of the types derived from specified base type, excluding the base type.</para>
    /// <para>This version is more flexible than its generic equivalent due to its lack of type parameter but it must go with <see cref="TypeScheduleTypeEditor.OptionsAttribute.BaseType"/> specified.</para>
    /// </summary>
    public class TypeScheduleTypeEditor : TypeScheduleTypeEditor<object>
    {
        protected Type BaseType;

        protected override object[] GetItems(ITypeDescriptorContext context)
        {
            var options = context.PropertyDescriptor.Attributes.OfType<TypeScheduleTypeEditor.OptionsAttribute>().First();
